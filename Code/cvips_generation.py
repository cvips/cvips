...
scenario = 'scenario00001'
root_path = "type1_subtype1_normal"
result_list = ['BEV_instance_camera', 'calib', 'label', 'lidar01']
result_list.extend(list(camera_positions_vehicle.keys()))
...
non_camera_subdirs = ['BEV_instance_camera', 'calib', 'label', 'lidar01']
# Loop over the town list
for town in town_list:
    # Loop over the vehicle list
...
class SynchronyModel(object):
    ...
    def current_captured_frame_num(self):
        ...
        label_path = os.path.join(root_path, vehicle_list[0], 'label', scene_name)
    ...
    def _make_setting(self):
        ...
        # cloudiness=20.0,  # Increase cloudiness
        # precipitation=60.0,  # Maximum precipitation to simulate heavy rain
        # precipitation_deposits=30.0,  # Increase water puddles on the road
        # wind_intensity=50.0,  # Some wind intensity for added realism
        # # sun_azimuth_angle=60.0,  # Set sun azimuth angle for daytime
        # # sun_altitude_angle=40.0  # Set sun altitude angle for high sun position
        # # sun_azimuth_angle=45.0,  # Sun position (not directly relevant to rain)
        # # sun_altitude_angle=-10.0  # Sun position (lower means less direct sunlight, can be overcast)
        # )
    ...
    def _span_sensor(self, player, player2, player3, player4):
        ...
        for camera_name, transform in camera_positions_vehicle.items():
            ...
            spawned_camera_d = self.world.spawn_actor(camera_d_bp,transform_d, attach_to=player,attachment_type=carla.AttachmentType.Rigid)
        ...
        for camera_name, transform in camera_positions_infra1.items():
            rsu_camera_transform = transform   # Modify this to adjust the location relative to RSU
        ...
        for camera_name, transform in camera_positions_infra2.items():
            rsuc_camera_transform = transform   # Modify this to adjust the location relative to RSU
    ...
    def _span_non_player(self):
        ...
        # breakpoint()
        # def is_spawn_point_occupied(spawn_point, existing_actors):
        #     for actor in existing_actors:
        #         if actor.get_location().distance(spawn_point.location) < 70:  # Define this threshold
        #             return True
        #     return False
        # motorcycle_blueprints = [x for x in self.world.get_blueprint_library().filter(FILTERV) if int(x.get_attribute('number_of_wheels')) == 2 and x.id in motorycle_name_list]
        # motorcycle_blueprints = sorted(motorcycle_blueprints, key=lambda bp: bp.id)

        # # Filtering for bicycles
        # bicycle_blueprints = [x for x in self.world.get_blueprint_library().filter(FILTERV) if int(x.get_attribute('number_of_wheels')) == 2 and x.id in bicycle_name_list]
        # bicycle_blueprints = sorted(bicycle_blueprints, key=lambda bp: bp.id)
        # # breakpoint()
        # available_spawn_points = [point for point in spawn_points if not is_spawn_point_occupied(point, vehicle_actors)]
        # NUM_OF_BICYCLES  = len(bicycle_blueprints)
        # NUM_OF_MOTORCYCLES = len(motorcycle_blueprints)
        # # breakpoint()
        # # Spawn Motorcycles
        # motorcycle_batch = []
        # for n, transform in enumerate(available_spawn_points):
        #     if n >= NUM_OF_MOTORCYCLES:
        #         break
        #     motorcycle_blueprint = random.choice(motorcycle_blueprints)
        #     motorcycle_batch.append(SpawnActor(motorcycle_blueprint, transform))

        # motorcycle_ids = []
        # for response in self.client.apply_batch_sync(motorcycle_batch):
        #     if response.error:
        #         logging.error(response.error)
        #     else:
        #         motorcycle_ids.append(response.actor_id)
        # motorcycle_actors = self.world.get_actors(motorcycle_ids)
        # self.non_player.extend(motorcycle_actors)
        # self.actor_list.extend(motorcycle_actors)

        # # Set to autopilot 
        # for i in motorcycle_actors:
        #     i.set_autopilot(True, self.traffic_manager.get_port())
        ...
        # bicycle_batch = []
        # for n, transform in enumerate(available_spawn_points):
        #     if n >= NUM_OF_BICYCLES:
        #         break
        #     bicycle_blueprint = random.choice(bicycle_blueprints)
        #     bicycle_batch.append(SpawnActor(bicycle_blueprint, transform))

        # bicycle_ids = []
        # for response in self.client.apply_batch_sync(bicycle_batch):
        #     if response.error:
        #         logging.error(response.error)
        #     else:
        #         bicycle_ids.append(response.actor_id)
        
        # bicycle_actors = self.world.get_actors(bicycle_ids)
        # self.non_player.extend(bicycle_actors)
        # self.actor_list.extend(bicycle_actors)       
        # for i in bicycle_actors:
        #     i.set_autopilot(True, self.traffic_manager.get_port())
    ...
    def _save_training_files(self, agent_vel_list, image_list, label_list, lidar_data_list, calib_list, depth_image_list):
        ...
        # breakpoint()
        for veh_idx,vehicle in enumerate(vehicle_list):   
            # print(vehicle)        
            scene_name = town + "_" + type_name + "_" + scenario
            # breakpoint()
            for img_idx, cam in enumerate(list(camera_positions_vehicle.keys())):
                img_path = os.path.join(root_path, vehicle, cam, scene_name)
            ...
            lidar_path = os.path.join(root_path, vehicle,'lidar01', scene_name, scene_name+"_"+str(self.captured_frame_no).zfill(3) + '.npz')
            calib_path = os.path.join(root_path, vehicle, 'calib', scene_name, scene_name+"_"+str(self.captured_frame_no).zfill(3) + '.pkl')
            bev_path = os.path.join(root_path, vehicle, 'BEV_instance_camera', scene_name, scene_name+"_"+str(self.captured_frame_no).zfill(3) + '.npz')
            label_path= os.path.join(root_path, vehicle, 'label', scene_name, scene_name+"_"+str(self.captured_frame_no).zfill(3) + '.txt')
...
def parse_args():
    ...
    argparser.add_argument("--town",  help="Town list: Town03, Town04, Town05, Town06, Town07, Town10HD", default="Town10HD")
    argparser.add_argument("--intersection", help="intersection type 4way or 3way", default="4way")
    argparser.add_argument("--setup", help="if 4way: wnse, wsee, wwss, wwee, wwws: 3way: wwss, wsse, wwse",default="4way")
    argparser.add_argument("--weather", help="weather type: clear, wet, cloudy, rainy", default="clear")    
    argparser.add_argument("--time_of_the_day", help="time of the day: noon, sunset, night",default="noon")
    argparser.add_argument("--crowd_level", help="crowd level: low, medium, high", default="medium")
